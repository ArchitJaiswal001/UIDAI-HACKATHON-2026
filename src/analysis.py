# =============================================================================
# FILE: analysis.py
# DESCRIPTION: UIDAI Hackathon Analysis - Data Loading, Processing & Visualization
# =============================================================================
...
search_path = "/content/" if os.path.exists("/content/") else os.getcwd()
...
def get_files(keyword):
    ...
    return glob.glob(f"{search_path}/**/*{keyword}*.csv", recursive=True)
...
for df in [df_enrol, df_bio, df_demo]:
    if not df.empty and {'date', 'state', 'district'}.issubset(df.columns):
...
grp_enrol = (
    df_enrol.groupby(['state', 'district', 'month'])
    [['age_0_5', 'age_5_17', 'age_18_greater']]
    .sum().reset_index()
) if not df_enrol.empty else pd.DataFrame()

grp_bio = (
    df_bio.groupby(['state', 'district', 'month'])
    [['bio_age_5_17', 'bio_age_17_']]
    .sum().reset_index()
) if not df_bio.empty else pd.DataFrame()

grp_demo = (
    df_demo.groupby(['state', 'district', 'month'])
    [['demo_age_17_']]
    .sum().reset_index()
) if not df_demo.empty else pd.DataFrame()
...
# --- ANNOTATION BLOCK FOR PUNE ---
# We find the exact value for Pune to place the arrow correctly
pune_row = top_migration[top_migration['district'] == 'Pune']

if not pune_row.empty:
    # Get the x-value (Ratio) and y-value (Index, usually 0 since it's top)
    pune_val = pune_row['migration_ratio'].values[0]
    pune_idx = 0  # Since we sorted descending, Pune is at index 0 (top bar)

    # Add the Annotation with an Arrow and a Text Box
    plt.annotate(
        '74k Updates vs 0 Enrolments:\nProves Labor Migration',  # The Text
        xy=(pune_val, pune_idx),                                 # Arrow Tip (End of Bar)
        xytext=(pune_val * 0.5, pune_idx + 3),                   # Text Position (Lower Right)
        arrowprops=dict(facecolor='black', shrink=0.05, width=2),# Arrow Style
        fontsize=11,
        fontweight='bold',
        color='black',
        bbox=dict(boxstyle="round,pad=0.3", fc="#fffacc", ec="black") # Yellow Box
    )
# ---------------------------------

plt.title('Top 10 "Migration Magnets" (High Address Updates vs New Enrolments)', fontsize=14, fontweight='bold')
plt.xlabel('Migration Flux Ratio (Updates per New Enrolment)', fontsize=12)
plt.ylabel('District', fontsize=12)
plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left') # Move legend outside to make room
plt.tight_layout()
plt.show()
...
if not df_demo.empty:
    daily_demo = (
        df_demo.groupby(['state', 'district', 'date'])
        ['demo_age_17_'].sum().reset_index()
    )

    stats = (
        daily_demo.groupby(['state', 'district'])
        ['demo_age_17_'].agg(['mean', 'std']).reset_index()
    )

    daily_demo = daily_demo.merge(stats, on=['state', 'district'])
    daily_demo = daily_demo[daily_demo['std'] > 0]

    daily_demo['z_score'] = (
        (daily_demo['demo_age_17_'] - daily_demo['mean']) / daily_demo['std']
    )

    anomalies = daily_demo.sort_values('z_score', ascending=False).head(1)

    if not anomalies.empty:
        top_state = anomalies.iloc[0]['state']
        top_district = anomalies.iloc[0]['district']

        subset = daily_demo[
            (daily_demo['state'] == top_state) &
            (daily_demo['district'] == top_district)
        ].sort_values('date')
        ...
        plt.annotate(
            f'CRITICAL SPIKE: {int(spike_val)}',
